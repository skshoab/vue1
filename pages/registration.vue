<template>
  <div class="capsule registration">

    <div >  <!--v-if="cartTotal > 0" -->
      <h1>Registration</h1>
    
      <div class="capsule name1">
       <label for="email">Email</label>
       <input id="email" type="email" v-model="stripeEmail" placeholder="name@example.com"/>
       <label for="email">Mobile</label>
       <input id="mobile" type="mobile" v-model="mobile" placeholder="017********"/>
       <label for="passward">passward</label>
       <input id="passward" type="passward" v-model="passward" placeholder="passward"/>
          <nuxt-link exact to="/cart"><button>Register</button></nuxt-link>
       </div>
        <!--<div class="capsule reg">
          <label for="registration">registration here </label>
          <nuxt-link exact to="/AppCheckout"><button>Registration</button></nuxt-link>
          </div>
       </div>-->
...
.name1 {
  border: 1px solid rgb(50, 123, 151);
  color: rgb(5, 4, 4);
  width: 500px;
  padding: 50px;
  display: flex;
